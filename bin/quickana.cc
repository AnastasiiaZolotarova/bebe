...
int main()
{
       ...
       */ // creation of an output is done
        ...
        for (Long64_t ien = 0; ien < nentries; ++ien)
             ...
             //  g_scatter->SetPoint(sc++, ien/2, fM);
               ...
               // p.pre_process(1000);
                ...
                ind_mean++;
                 ...
                 //---------------Shape parameter using mean walue---------------
                  ...
                  for (size_t in = start_index; in < p.n_samples(); ++in)
                  {
                      s1 +=data[in]*model[in];
                      s2 +=data[in]; 
                  }
                ...
                //---------------Shape parameter using integral---------------
                 ...
                 float s_l=0, s_h=0, m_l=0,m_h=0;
                  ...
                  for (size_t in = (start_index+(p.n_samples()-start_index)/4); in < p.n_samples(); ++in)
                  {
                    s_h+=data[in]/fM; 
                    m_h+=model[in]; 
                  }
                ...
                */
                 if (fM > 0 && fM < 12000 && SI_int>-5 && SI_int<5 ) 
                       g_shape_ampl_int->SetPoint(gcnt2++, fM, SI_int);
  ...
  //--------------RUN SECOND TIME WITH MEAN PULSE COLLECTED-----------
        double  model1;
         fclose(fp);
         fp=fopen("pulses.dat","w+");
      //  model1=p_average_signal->GetArray();
